...
df = spark.read.csv("csv/online_retail_clean.csv", header=True, inferSchema=True)
...
ticket_promedio = df.groupBy("InvoiceNo")\
    .agg(_sum(col("Quantity")* col("UnitPrice")).alias("total_factura"))\
...
print("Resultados ETL guardados en la carpeta out 'out/'")
